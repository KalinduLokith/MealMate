...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val database = Room.databaseBuilder(this, DBConnection::class.java, "DataBase").build()
        val mealDao = database.databaseDao()


        var addMealToDb = findViewById<Button>(R.id.btn_add_meals)
        var searchForMealsByIngredient = findViewById<Button>(R.id.btn_search_by_ingredient)
        var searchForMeals = findViewById<Button>(R.id.btn_search_meals)
        var SearchMealsByWeb=findViewById<Button>(R.id.btn_search_meals_by_web)


        addMealToDb.setOnClickListener {
          addMealsToDb(mealDao)
        }



        searchForMealsByIngredient.setOnClickListener {
            val dataPass=database.openHelper.databaseName
            val searchForMealsByIngredient = Intent(this, IngredientSearch::class.java).apply { putExtra("dataCollect",dataPass) }
            startActivity(searchForMealsByIngredient)
        }

        searchForMeals.setOnClickListener {
            val dataPass=database.openHelper.databaseName
            val searchForMealsByIngredient1 = Intent(this, SearchByMeals::class.java).apply { putExtra("db_id",dataPass) }
            startActivity(searchForMealsByIngredient1)
        }
        SearchMealsByWeb.setOnClickListener{
            val searchForMealsByIngredient2 = Intent(this, SearchMealByWeb::class.java)
            startActivity(searchForMealsByIngredient2)
        }
    }

    private fun addMealsToDb(mealDao : MealDao) {

        runBlocking {
            launch {
                val firstMeal = DataBase(
                    1,
                    "Sweet and Sour Pork",
                null,
                "Pork",
            ...
            val fourthMeal = DataBase(
                ...
                val mealList : List<DataBase> =  mealDao.getAll();
                for (meals in mealList) {
                    stringBuilder.append("\n ${meals.idMeal } ${meals.strCategory}")
